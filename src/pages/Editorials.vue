      ...
      <table class="w-full flex flex-row flex-no-wrap overflow-hidden my-5 pl-4 pr-4">
        ...
        <tbody class="flex-1 sm:flex-none">
          ...
          class="flex flex-col flex-no wrap sm:table-row sm:mb-0 relative border-b sm:border-b-0">
            <td class="p-2 md:p-4 text-md md:pl-0 md:text-3xl"><g-link class="block w-full h-full" :to="editorial.node.path">{{ editorial.node.title }}</g-link></td>
            <td class="p-2 md:p-4 text-md md:pl-0 md:text-3xl"><g-link class="block w-full h-full italic" :to="editorial.node.path">{{ editorial.node.publication }}</g-link></td>
            <td class="p-2 md:p-4 text-md md:pl-0 md:text-3xl"><g-link class="block w-full h-full" :to="editorial.node.path">{{ editorial.node.date }}</g-link></td>
          </tr>
        </tbody>
      </table>
    </div>
