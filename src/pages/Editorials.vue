<template>
  <Layout>
   <div class="flex flex-col">
     <table class="w-full flex flex-row flex-no-wrap overflow-hidden my-5 border-b">
			...
			<tbody class="flex-1 sm:flex-none">
				<tr v-for="editorial in $page.editorial.edges" :key="editorial.id" class="flex flex-col flex-no wrap sm:table-row mb-2 sm:mb-0 border-b relative">
					<td class="p-4"><g-link class="block w-full h-full" :to="editorial.node.path">{{ editorial.node.title }}</g-link></td>
					<td class="p-4"><g-link class="block w-full h-full" :to="editorial.node.path">{{ editorial.node.publication }}</g-link></td>
					<td class="p-4"><g-link class="block w-full h-full" :to="editorial.node.path">{{ editorial.node.date }}</g-link></td>
				</tr>
			</tbody>
		</table>
    </div>
    ...
    <ul>
      <li v-for="editorial in $page.editorial.edges" :key="editorial.id">
        <g-link :to="editorial.node.path"> {{ editorial.node.title }} </g-link>
      </li>
    </ul>
  </Layout>
</template>
